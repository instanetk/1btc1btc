...
async function pollUpvotes() {
  try {
    const latestBlock = await client.getBlockNumber();

    // On first run, start from the current block (only notify on new events)
    if (lastProcessedBlock === 0n) {
      lastProcessedBlock = latestBlock;
      console.log(`[UpvoteListener] Initialized at block ${latestBlock}`);
      return;
    }
    ...
    console.log(`[UpvoteListener] Checking blocks ${fromBlock}–${latestBlock}`);
    ...
    // Advance cursor even if no logs
    lastProcessedBlock = latestBlock;

    if (logs.length === 0) return;

    console.log(`[UpvoteListener] Found ${logs.length} Upvoted event(s)`);
    ...
    console.error("[UpvoteListener] Poll error:", error);
  }
}
...
export function startUpvoteListener() {
  ...
  pollUpvotes();
  setInterval(pollUpvotes, POLL_INTERVAL_MS);
}
