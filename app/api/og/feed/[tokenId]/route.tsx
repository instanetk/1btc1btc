...
// Bitcoin logo SVG — same paths as contract, repositioned for OG
const LOGO =
  '<svg x="525" y="50" width="150" height="73" viewBox="170 330 685 332">' +
...
function buildOrbital(
  ...
  const bits = Number((h >> BigInt(idx * 32)) & 0xffffffffn);
  const rot = bits % 360;
  const pos = (bits >> 9) % 8;
  ...
  if (pos === 0) {
    ...
    dotX = CX + Math.round((rx * 707) / 1000);
    dotY = CY + Math.round((ry * 707) / 1000);
  } else if (pos === 2) {
    dotX = CX;
    dotY = CY + ry;
  } else if (pos === 3) {
  ...
  return (
    ...
    `<ellipse cx="${CX}" cy="${CY}" rx="${rx}" ry="${ry}" fill="none" stroke="#F7931A" stroke-width="1" opacity="0.15"/>` +
    `<circle cx="${dotX}" cy="${dotY}" r="4" fill="#F7931A" opacity="0.4"/>` +
    `</g>`
  );
}

function buildOrbitals(tokenId: number): string {
  const h = BigInt(
    keccak256(encodePacked(["uint256"], [BigInt(tokenId)]))
  );
  return [
    buildOrbital(h, 0, 420, 180),
    buildOrbital(h, 1, 340, 140),
    buildOrbital(h, 2, 500, 200),
    buildOrbital(h, 3, 260, 100),
  ].join("");
}

function buildAnalogyText(text: string): string {
  ...
  const lineHeight = 46;
  const textAreaTop = 170;
  const textAreaBottom = 580;
  ...
  if (totalTextHeight >= textAreaHeight) {
    startY = textAreaTop + 32;
  } else {
    startY = textAreaTop + (textAreaHeight - totalTextHeight) / 2 + 32;
  }

  const tspans = lines
    .map((line, i) => {
      if (i === 0) {
        return `<tspan x="${CX}" y="${Math.round(startY)}">${line}</tspan>`;
      }
      return `<tspan x="${CX}" dy="${lineHeight}">${line}</tspan>`;
    })
    .join("");

  return `<text text-anchor="middle" fill="#F5F0E8" font-family="Georgia, 'Times New Roman', serif" font-size="36">${tspans}</text>`;
}

function buildSvg(tokenId: number, text: string): string {
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">` +
    `<rect width="${W}" height="${H}" fill="#0A0A0A"/>` +
    `<rect x="12" y="12" width="${W - 24}" height="${H - 24}" rx="8" fill="none" stroke="#FFF" stroke-opacity="0.06"/>` +
    `<circle cx="${CX}" cy="${CY}" r="250" fill="#F7931A" opacity="0.02"/>` +
    `<circle cx="${CX}" cy="${CY}" r="400" fill="#F7931A" opacity="0.01"/>` +
    buildOrbitals(tokenId) +
    LOGO +
    `<text x="${CX}" y="145" text-anchor="middle" fill="#F7931A" opacity="0.5" font-family="sans-serif" font-size="16" textLength="180" lengthAdjust="spacing">1 BTC = 1 BTC</text>` +
    buildAnalogyText(text) +
    `<text x="50" y="604" fill="#555" font-family="sans-serif" font-size="14">#${tokenId}</text>` +
    `<text x="${W - 50}" y="604" text-anchor="end" fill="#555" font-family="sans-serif" font-size="14">1BTC1BTC.money</text>` +
    `</svg>`
  );
}
